...
class FirstAdminSetup {
  ...
  sha256Fallback(ascii) {
    ...
    var result = "";
    var words = [];
    var msg = toUtf8(ascii);
    var msgBitLen = msg.length * 8;
    var H = [],
      K = [],
      isComposite = {};
    for (var candidate = 2; K.length < 64; candidate++) {
      if (!isComposite[candidate]) {
        for (i = candidate * candidate; i <= 313; i += candidate)
          isComposite[i] = true;
        H.push((Math.pow(candidate, 0.5) * maxWord) | 0);
    ...
    words[words.length] = (msgBitLen / maxWord) | 0;
    words[words.length] = msgBitLen;
    var w = new Array(64);
    var hash = H.slice(0);
    for (j = 0; j < words.length; ) {
      var block = words.slice(j, (j += 16));
      for (i = 0; i < 64; i++) {
        var w15 = w[i - 15],
          w2 = w[i - 2];
        w[i] =
        ...
        var t2 =
          ((rotr(2, a) ^ rotr(13, a) ^ rotr(22, a)) +
            ((a & b) ^ (a & c) ^ (b & c))) |
          0;
        hash = [(t1 + t2) | 0, a, b, c, (d + t1) | 0, e, f, g];
      }
      for (i = 0; i < 8; i++) H[i] = (H[i] + hash[i]) | 0;
      hash = H.slice(0);
    }
    for (i = 0; i < 8; i++) {
      for (j = 3; j + 1; j--) {
