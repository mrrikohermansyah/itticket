...
// Bulk resolve helper
window.bulkResolveTicketsByDateRange = async function (
  startDateStr,
  endDateStr
) {
  try {
    ...
    for (const docSnap of docs) {
      ...
      const isProgress =
        ...
        s1 === "in progress" ||
        s2 === "in progress" ||
        s1.includes("progress") ||
        s2.includes("progress");
...
window.migrateDepartmentToUserDepartmentRange = async function (
  startDateStr,
  endDateStr
) {
  try {
    ...
    for (const docSnap of docs) {
      ...
      if (newUserDep === currentUserDep) {
        skipped++;
        continue;
      }
      await updateDoc(doc(db, "tickets", docSnap.id), {
        user_department: newUserDep,
        last_updated: serverTimestamp(),
      });
      updated++;
    }
...
window.migrateAssignmentLocationRange = async function (
  startDateStr,
  endDateStr
) {
  try {
    ...
    for (const docSnap of docs) {
      ...
      if (typeof d.message === "string" && d.message.includes("Location:")) {
        updatePayload.message = d.message.replace(
          /(Location:\s*)([^|]+)(?=\s*\|?|$)/,
          `$1${newLocation}`
        );
      }
      if (
        typeof d.assignment_message === "string" &&
        d.assignment_message.includes("Location:")
      ) {
        updatePayload.assignment_message = d.assignment_message.replace(
          /(Location:\s*)([^|]+)(?=\s*\|?|$)/,
          `$1${newLocation}`
        );
      }
...
window.normalizeTicketCodes = async function () {
  try {
    ...
    const gen =
      typeof window !== "undefined" ? window.generateTicketId : undefined;
    let updated = 0;
    for (const docSnap of snap.docs) {
      const data = docSnap.data();
      if (
        typeof data.code === "string" &&
        (data.code.startsWith("TKT-") ||
          data.code.indexOf("-") === -1 ||
          data.code.length <= 3)
      ) {
        ...
        const ts = data.created_at?.toDate
          ...
          : new Date();
        const newCode = gen
          ? gen(dept, device, location, docSnap.id, ts)
          : `${dept}-${location}-${device}`;
  ...
  try {
    ...
    for (const docSnap of snap.docs) {
      ...
      let newStatus =
        data.status ||
        data.status_ticket ||
        (data.qa === "Finish" ? "Resolved" : data.qa || "Open");
...
class AdminDashboard {
  ...
  bindMethods() {
    ...
    this.updateBulkDeleteVisibility =
      this.updateBulkDeleteVisibility.bind(this);
      ...
      notification.innerHTML = `
                <div class="notification-content">
                    ...
                    <div class="notification-text">
                        ...
                        ${
                          message
                            ? `<br><span style="font-size: 0.9rem; opacity: 0.9;">${message}</span>`
                            : ""
                        }
                    </div>
      ...
      if (adminSnap.exists()) {
        ...
        try {
          ...
          if (!byEmail.empty) {
            ...
            try {
              await deleteDoc(doc(this.db, "admins", found.id));
            } catch (e) {}
            this.adminUser = { uid: firebaseUser.uid, ...data };
            console.info("Current admin user:", {
  ...
  canTakeTicket(ticket) {
    const scope = (
      ticket &&
      (ticket.assignment_scope || ticket.scope || "")
    ).toLowerCase();
    if (ticket && ticket.is_assignment && scope === "all") {
      const alreadyTakenByMe =
        Array.isArray(ticket.taken_by) &&
        ticket.taken_by.includes(this.adminUser?.uid);
  ...
  canReleaseTicket(ticket) {
    if (this.adminUser.role === "Super Admin") return true;
    return (
      this.isAssignedToCurrentAdmin(ticket) && ticket.status !== "Resolved"
    );
  }
  ...
  isAssignedToCurrentAdmin(ticket) {
    ...
    if (
      ticket.target_admin_uid &&
      ticket.target_admin_uid === this.adminUser.uid
    ) {
      return true;
    }
    ...
    if (
      this.adminUser.name &&
      (ticket.action_by === this.adminUser.name ||
        ticket.assigned_to === this.adminUser.name)
    ) {
      return true;
    }
    ...
    if (
      ...
      (ticket.action_by === this.adminUser.email ||
        ticket.assigned_to === this.adminUser.email ||
        ticket.target_admin_email === this.adminUser.email)
    ) {
      return true;
    }
  ...
  initializeEventListeners() {
    ...
    if (selectAll) {
      selectAll.addEventListener("change", (e) =>
        this.selectAllVisibleTickets(e.target.checked)
      );
    }
  ...
  initializeDateFilter() {
    ...
    if (thisMonthBtn)
      thisMonthBtn.addEventListener("click", this.handleThisMonthClick);
    if (clearDateBtn)
  ...
  applyAllFilters() {
    ...
    if (this.currentFilters.status !== "all") {
      filtered = filtered.filter(
        (ticket) => ticket.status === this.currentFilters.status
      );
    }
    ...
    if (this.currentFilters.priority !== "all") {
      filtered = filtered.filter(
        (ticket) => ticket.priority === this.currentFilters.priority
      );
    }
  ...
  persistFilters() {
    try {
      const payload = {
        ...
        date: {
          startDate: this.currentFilters.date.startDate
            ? this.currentFilters.date.startDate.toISOString()
            : null,
          endDate: this.currentFilters.date.endDate
            ? this.currentFilters.date.endDate.toISOString()
            : null,
  ...
  setFilterUIFromState() {
    try {
      ...
      if (statusFilter && this.currentFilters.status)
        statusFilter.value = this.currentFilters.status;
      if (priorityFilter && this.currentFilters.priority)
        priorityFilter.value = this.currentFilters.priority;

      if (startDateInput && this.currentFilters.date.startDate) {
        const d = new Date(
          this.currentFilters.date.startDate.getTime() -
            this.currentFilters.date.startDate.getTimezoneOffset() * 60000
        );
        startDateInput.value = d.toISOString().split("T")[0];
      }
      if (endDateInput && this.currentFilters.date.endDate) {
        const d = new Date(
          this.currentFilters.date.endDate.getTime() -
            this.currentFilters.date.endDate.getTimezoneOffset() * 60000
        );
  ...
  handleThisMonthClick() {
    ...
    if (startDateInput && endDateInput) {
      ...
      this.showNotification(
        "Date Filter",
        "info",
        "Showing tickets for this month"
      );
    }
  }
  ...
  async loadTickets() {
    try {
      ...
      try {
        ...
        console.warn(
          "Fallback tickets query without orderBy due to error:",
          e?.message || e
        );
      ...
      if (!querySnapshot || querySnapshot.docs.length === 0) {
        ...
        try {
          if (this.adminUser?.uid)
            await firebaseAuthService.ensureAdminRecord(
              this.adminUser.uid,
              this.adminUser.email || ""
            );
        } catch (_) {}
      ...
      const broadcastsToCascade = allTickets.filter(
        (t) =>
          t.is_assignment &&
          (t.assignment_scope || t.scope || "").toLowerCase() === "all" &&
          t.deleted &&
          !t.cascade_deleted_children
      );
      if (broadcastsToCascade.length > 0) {
        for (const b of broadcastsToCascade) {
          try {
            const cq = query(
              collection(this.db, "tickets"),
              where("source_assignment_id", "==", b.id)
            );
      ...
      if (
        ...
        try {
          ...
          const queries = [
            ...
            query(
              collection(this.db, "tickets"),
              where("target_admin_uid", "==", uid)
            ),
            ...(email
              ? [
                  query(
                    collection(this.db, "tickets"),
                    where("target_admin_email", "==", email)
                  ),
                ]
              : []),
          ];
          ...
          if (targeted.length > 0) {
            ...
            this.tickets = Array.from(map.values()).sort(
              (a, b) =>
      ...
      try {
        if (this.adminUser?.uid) {
          const ok = await firebaseAuthService.ensureAdminRecord(
            this.adminUser.uid,
            this.adminUser.email || ""
          );
          if (ok) {
  ...
  async loadMoreTickets() {
    try {
      ...
      const snap = await getDocs(
        query(baseQuery, startAfter(this.lastVisible), limit(this.pageSize))
      );
      const newTickets = [];
  ...
  normalizeTicketData(id, data) {
    try {
      const created_at =
        this.parseDateToISO(data.created_at) ||
        this.parseDateToISO(data.createdAt) ||
        new Date().toISOString();
      const last_updated =
        this.parseDateToISO(data.last_updated) ||
        this.parseDateToISO(data.updatedAt) ||
        created_at;
      ...
      return {
        ...
        subject: data.subject || data.assignment_subject || "No Subject",
        user_name: data.user_name || "Unknown User",
        user_email: data.user_email || "",
        user_department: data.user_department || "",
        location: data.location || data.assignment_location || "",
        ...
        created_by_name:
          ...
          data.user_name ||
          (data.created_by_email || "").split("@")[0] ||
          "",
        created_by_email: data.created_by_email || "",
        created_by_uid: data.created_by_uid || "",
        inventory: data.inventory || "",
        device: data.device || "",
        message: data.message || data.assignment_message || "",
  ...
  async loadAssignments() {
    try {
      ...
      const mineTicketsQ = query(
        collection(this.db, "tickets"),
        where("is_assignment", "==", true),
        where("target_admin_uid", "==", this.adminUser.uid)
      );
      const mineTicketsEmailQ = query(
        collection(this.db, "tickets"),
        where("is_assignment", "==", true),
        where("target_admin_email", "==", this.adminUser.email)
      );
      ...
      const [mt, mte, at] = await Promise.all([
        getDocs(mineTicketsQ),
        getDocs(mineTicketsEmailQ),
        getDocs(allTicketsQ),
      ]);
      const items = [];
      ...
      const filtered = uniqueItems.filter(
        (a) =>
          a.scope === "all" ||
          a.target_uid === this.adminUser.uid ||
          a.target_email === this.adminUser.email
      );
      // Sort newest first
      filtered.sort(
        (a, b) =>
  ...
  setupAssignmentsRealtime() {
    try {
      ...
      this.assignmentsUnsubMineEmail = onSnapshot(
        mineTicketsEmailQ,
        handleEmailSnap
      );
  ...
  mergeAssignments(newItems) {
    ...
    this.assignments = filtered.sort(
      ...
      .map((a) => {
        ...
        return `
          ...
          a.priority || "Medium"
                        ...
                        ${
                          subject
                            ? `<div class="ticket-meta"><strong>Subject:</strong> ${subject}</div>`
                            : ""
                        }
                        ${
                          message
                            ? `<div class="ticket-meta"><strong>Message:</strong> ${message}</div>`
                            : ""
                        }
                        <div class="ticket-meta">
                            <span class="ticket-date">${date}</span>
                            <span class="ticket-device">Created by: ${
                              (a.created_by_name || "").toLowerCase() === "it"
                                ? "IT"
                                : a.created_by_name || ""
                            }</span>
                        </div>
  ...
  setupTicketsRealTimeListener() {
    try {
      ...
      try {
          ...
          (snapshot) => {
            ...
            this.lastVisible =
              snapshot.docs.length > 0
                ...
                : null;
            this.hasMoreTickets =
              !this.isShowingAll && snapshot.docs.length === this.pageSize;
            this.applyAllFilters();
            this.updatePaginationControls();
          },
          (error) => {
            console.warn(
              "Realtime listener error, switching to fallback:",
              error?.message || error
            );
            ...
            this.unsubscribe = onSnapshot(
              ...
              (snap) => {
                ...
                snap.forEach((ds) => {
                  try {
                    arr.push(this.normalizeTicketData(ds.id, ds.data()));
                  } catch {}
                });
                arr.sort(
                  (a, b) =>
        ...
        console.warn(
          "Realtime listener orderBy failed, using collection fallback:",
          err?.message || err
        );
        this.unsubscribe = onSnapshot(
          collection(this.db, "tickets"),
  ...
  updateAdminAssignmentsInRealTime(adminId, adminData) {
    try {
      const affectedTickets = this.tickets.filter(
        (t) => t.assigned_to === adminId || t.action_by === adminId
      );
      if (affectedTickets.length > 0) {
        const name =
          adminData.name ||
          (window.adminCache && window.adminCache[adminId]?.name) ||
          "";
        const email =
          adminData.email ||
          (window.adminCache && window.adminCache[adminId]?.email) ||
          "";
  ...
  updateUserTicketsInRealTime(userId, userData) {
    try {
      ...
      const userTickets = this.tickets.filter(
        (ticket) =>
          ticket.user_id === userId || ticket.user_email === userData.email
      );
  ...
  async renderTickets() {
    ...
    const rowPromises = this.filteredTickets.map(async (ticket) => {
      ...
      const assignedAdminDisplay = await this.getAssignedAdminDisplayInfo(
        ticket
      );
      ...
      const assignedEmail =
        ...
        this.adminUser.email
          ...
          assignedInfo &&
            ...
            assignedInfo.assignedTo.email
          ? assignedInfo.assignedTo.email
          : // 3) If assigned_to already an email string
          isEmail(ticket.assigned_to)
      ...
      return `
                <tr data-ticket-id="${ticket.id}">
                    <td style="text-align:center;">
                        <input type="checkbox" class="ticket-select" data-ticket-id="${
                          ticket.id
                        }" ${
        this.selectedTickets.has(ticket.id) ? "checked" : ""
      } aria-label="Select ticket ${ticket.code || ""}">
                    </td>
                        ...
                        ${
                          ticket.is_assignment
                            ...
                            <div class=\"assignment-inline-meta\">
                                <small>${
                                  (
                                    ticket.assignment_scope || ""
                                  ).toLowerCase() === "all"
                                    ? "All IT"
                                    : "Assignment"
                                } • Created by: ${this.escapeHtml(
                    ...
                    <td>
                        <div class="action-buttons" data-ticket-id="${
                          ticket.id
                        }">
                            ${actionButtons}
                        </div>
    ...
    if (selectAll) {
      ...
      const allSelected =
        visibleIds.length > 0 &&
        visibleIds.every((id) => this.selectedTickets.has(id));
  ...
  handleSelectionChange(e) {
    ...
    if (selectAll) {
      ...
      const allSelected =
        visibleIds.length > 0 &&
        visibleIds.every((i) => this.selectedTickets.has(i));
      selectAll.checked = allSelected;
    }
  ...
  updateBulkDeleteVisibility() {
    ...
    const hasVisibleSelected =
      Array.isArray(this.filteredTickets) &&
      this.filteredTickets.some((t) => this.selectedTickets.has(t.id));
  ...
  generateActionButtons(ticket, permissions) {
    ...
    // Take Ticket button
    const isAllAssignment =
      ticket.is_assignment &&
      (ticket.assignment_scope || ticket.scope || "").toLowerCase() === "all";
    ...
    if (
      (permissions.isTicketOwner || permissions.isSuperAdmin) &&
    ...
    const cardPromises = this.filteredTickets.map(async (ticket) => {
      ...
      return `
                <div class="ticket-card" data-ticket-id="${ticket.id}">
                    ...
                    <div class="ticket-title">${escapeHTML(
                      ticket.subject || "No Subject"
                    )}</div>
                    ${
                      ticket.is_assignment
                    ...
                    <div class=\"ticket-assignment-meta\">
                        <small>${
                          (ticket.assignment_scope || "").toLowerCase() ===
                          "all"
                            ? "All IT"
                            : "Assignment"
                    ...
                    <div class="ticket-user-info">
                        <div class="ticket-user">${
                          userDisplay.name || "Unknown"
                        }</div>
                        <div class="ticket-department">${
                          userDisplay.department || "N/A"
                        }</div>
                    </div>
                    ...
                    <div class="ticket-meta">
                        ...
                        <span class="status-badge status-${(
                          ...
                          .toLowerCase()
                          .replace(" ", "-")}">
                            <i class="fas fa-${this.getStatusIcon(
                              ticket.status
                            )}"></i> 
                            ${ticket.status || "Open"}
                        </span>
  ...
  async executeStatusUpdate(ticketId, newStatus, ticket) {
    ...
    const updateNote = {
      status: newStatus,
      notes: `Status changed to ${newStatus} by ${
        this.adminUser.name || this.adminUser.email
      }`,
    ...
    const currentUpdates = Array.isArray(currentData.updates)
      ? currentData.updates
      : [];
  ...
  async takeTicket(ticketId) {
    try {
      ...
      if (isAllAssignment) {
        const subject =
          ticket.assignment_subject ||
          ticket.subject ||
          ticket.assignment_activity ||
          ticket.activity ||
          "Assignment";
        ...
        const newDocRef = await addDoc(collection(this.db, "tickets"), {
          ...
          is_assignment: false,
          assignment_activity:
            ticket.assignment_activity || ticket.activity || "",
        });
        ...
        if (typeof gen === "function") {
          const code = gen(
            this.adminUser.department || "IT",
            ticket.device || "Others",
            location,
            newDocRef.id,
            ticket.assignment_activity || ticket.activity || ""
          );
  ...
  async releaseTicket(ticketId) {
    try {
      ...
      this.showNotification(
        "Ticket Released",
        "success",
        "Ticket is now available for other admins"
      );
      await this.loadTickets();
    } catch (error) {
    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        if (isBroadcast) {
          const q = query(
            collection(this.db, "tickets"),
            where("source_assignment_id", "==", ticketId)
          );
          const snap = await getDocs(q);
          const deletions = [];
          snap.forEach((ds) =>
            deletions.push(deleteDoc(doc(this.db, "tickets", ds.id)))
          );
  ...
  async showResolveConfirmation(ticket) {
    try {
        ...
        html: `
                    <div class="resolve-modal">
                        <div class="ticket-info">
                            <p><strong>Subject:</strong> ${this.escapeHtml(
                              ticket.subject
                            )}</p>
                            <p><strong>User:</strong> ${this.escapeHtml(
                              ticket.user_name
                            )}</p>
                            <p><strong>Current Status:</strong> ${this.escapeHtml(
                              ticket.status
                            )}</p>
                        </div>
        ...
        preConfirm: () => {
          ...
          if (!note) {
            Swal.showValidationMessage(
              "⚠️ Resolution notes are REQUIRED before resolving a ticket"
            );
            return false;
          }
          ...
          if (note.length < 10) {
            Swal.showValidationMessage(
              "⚠️ Please provide more detailed notes (minimum 10 characters)"
            );
            return false;
          }
      ...
      if (formValues) {
        await this.executeTicketResolution(
          ticket.id,
          formValues.status,
          formValues.note
        );
  ...
  async executeTicketResolution(ticketId, newStatus, note) {
    try {
      ...
      const finalStatuses = ["Resolved", "Closed", "Completed", "Finished"];
      const isCurrentlyFinalStatus = finalStatuses.includes(
        currentData?.status
      );
      ...
      this.showNotification(
        "Ticket Resolved",
        "success",
        "Ticket has been successfully resolved"
      );
      await this.loadTickets();
    } catch (error) {
  ...
  setupTicketRealTimeListener(ticketId, modalBody) {
    try {
      ...
      this.ticketModalUnsubscribe = onSnapshot(
        ticketRef,
        async (docSnapshot) => {
          try {
            ...
            if (ticket.user_id) {
              try {
                const userDoc = await getDoc(
                  doc(this.db, "users", ticket.user_id)
                );
                if (userDoc.exists()) {
                  ...
                  ticket.user_department =
                    userProfile.department || ticket.user_department;
                }
              } catch (e) {
            ...
            if (!this.ticketUserModalUnsubscribe && ticket.user_id) {
              ...
              this.ticketUserModalUnsubscribe = onSnapshot(
                userRef,
                (userSnap) => {
                  if (!userSnap.exists()) return;
                  const user = userSnap.data();
                  const nameEl = modalBody.querySelector(
                    '[data-field="user_name"]'
                  );
                  const emailEl = modalBody.querySelector(
                    '[data-field="user_email"]'
                  );
                  const deptEl = modalBody.querySelector(
                    '[data-field="user_department"]'
                  );
                  if (nameEl)
                    nameEl.textContent = this.escapeHtml(
                      user.full_name || "Unknown User"
                    );
                  if (emailEl)
                    emailEl.textContent = this.escapeHtml(user.email || "");
                  if (deptEl)
                    deptEl.textContent = this.escapeHtml(user.department || "");
                },
                (err) => {
  ...
  async renderRealTimeModalContent(ticket, modalBody) {
    try {
      ...
      const permissions = this.checkPermissions(ticket);
      const userDisplay = await this.getUserDisplayInfo(ticket);
      const modalHTML = this.getTicketModalHTML(
        ticket,
        assignedAdmins,
        permissions,
        userDisplay
  ...
  getTicketModalHTML(ticket, assignedAdmins, permissions, userDisplay) {
    ...
    const resolvedDisplay = resolvedDate
        ...
        <div class="ticket-details real-time-ticket">
            ...
            <div class="ticket-real-time-header">
                <div class="ticket-summary-bar">
                    <span class="ticket-code">${this.escapeHtml(
                      ticket.code
                    )}</span>
            ...
            <div class="ticket-section">
                ...
                <div class="ticket-row">
                    ...
                    <div class="ticket-col value" data-field="subject">${this.escapeHtml(
                      ticket.subject
                    )}</div>
                </div>
                <div class="ticket-row">
                    <div class="ticket-col"><strong>Message:</strong></div>
                    <div class="ticket-col value" data-field="message">${this.escapeHtml(
                      ticket.message
                    )}</div>
                </div>
                <div class="ticket-row">
                    <div class="ticket-col"><strong>Activity:</strong></div>
                    <div class="ticket-col value" data-field="activity">${this.escapeHtml(
                      ticket.assignment_activity || ticket.activity || "-"
                    )}</div>
                </div>
                <div class="ticket-row">
                ...
                <div class="ticket-row">
                    ...
                    <div class="ticket-col value" data-field="location">
                        ${this.escapeHtml(
                          ticket.assignment_location || ticket.location || "-"
                        )}
                    </div>
                </div>
                <div class="ticket-row">
            ...
            <div class="ticket-section">
                ...
                <div class="ticket-row">
                    ...
                    <div class="ticket-col value" data-field="user_name">${this.escapeHtml(
                      userDisplay.name
                    )}</div>
                </div>
                <div class="ticket-row">
                    <div class="ticket-col"><strong>Department:</strong></div>
                    <div class="ticket-col value" data-field="user_department">${this.escapeHtml(
                      userDisplay.department || "N/A"
                    )}</div>
                </div>
                <div class="ticket-row">
                    <div class="ticket-col"><strong>Email:</strong></div>
                    <div class="ticket-col value" data-field="user_email">${this.escapeHtml(
                      userDisplay.email || ""
                    )}</div>
                </div>
            </div>
                    ...
                    <div class="ticket-col value" data-field="assigned_to">
                        ${
                          assignedAdmins.assignedTo
                            ? `${this.escapeHtml(
                                assignedAdmins.assignedTo.name
                              )} 
                            <small>(${this.escapeHtml(
                              assignedAdmins.assignedTo.email
                            )})</small>`
                            : "<em>Unassigned</em>"
                        }
                    </div>
                      ...
                      ticket.updates && ticket.updates.length > 0
                        ? ticket.updates
                            ...
                            <div class="update-item">
                                ...
                                <div class="update-notes">${this.escapeHtml(
                                  update.notes || "–"
                                )}</div>
                                <div class="update-by">by ${this.escapeHtml(
  ...
  async handleExportWithCustomDialog() {
    try {
      ...
      if (exportBtn) {
        exportBtn.dataset.originalHtml = originalText;
      ...
      if (exportAction === false) {
        ...
        if (typeof window.appendToExistingExcel === "function") {
          try {
            await window.appendToExistingExcel(
              exportData,
        ...
        if (typeof window.createNewFileWithTemplate === "function") {
          try {
            await window.createNewFileWithTemplate(
              exportData,
              this.getCurrentFilterInfo()
            );
            exportSuccess = true;
      ...
      const exportBtn = document.getElementById("exportTickets");
      if (exportBtn) {
        const restore = exportBtn.dataset.originalHtml;
        exportBtn.innerHTML = restore
          ? restore
          : '<i class="fas fa-file-excel"></i><span class="btn-text">Export Excel</span>';
    ...
    try {
      ...
      if (typeof window.originalExportToExcelAppendSorted === "undefined") {
        if (typeof window.exportToExcelAppendSorted === "function") {
          window.originalExportToExcelAppendSorted =
            window.exportToExcelAppendSorted;
        } else {
          window.originalExportToExcelAppendSorted = function (
            displayedTickets,
            filterInfo
          ) {
            console.warn(
              "⚠️ originalExportToExcelAppendSorted called but not implemented"
            );
      ...
      if (typeof window.appendToExistingExcel === "undefined") {
        window.appendToExistingExcel = async function (
          ...
          if (typeof window.originalExportToExcelAppendSorted === "function") {
            return window.originalExportToExcelAppendSorted(
              displayedTickets,
              filterInfo
            );
          }
    ...
    try {
      const exportTickets = this.filteredTickets.map((ticket) => {
        ...
        if (ticket.assigned_to) {
          const adminInfo =
            window.adminCache && window.adminCache[ticket.assigned_to];
          if (adminInfo) {
          ...
          const adminInfo =
            window.adminCache && window.adminCache[ticket.action_by];
          if (adminInfo) {
            assignedAdmin = `${adminInfo.name} (${adminInfo.email})`;
          } else {
        ...
        const cachedUser =
          ticket.user_id && window.userCache
            ? window.userCache[ticket.user_id]
            : null;
        const resolvedFullName =
          (cachedUser && cachedUser.full_name) ||
          ticket.user_name ||
          "Unknown User";
        ...
        if (actLower === "deliver") activityCode = "MV";
          ...
          actLower === "software install" ||
          actLower === "software config" ||
          actLower === "install it standard apps" ||
          actLower === "reinstall windows"
        )
          activityCode = "SW";
        else if (actLower === "setup meeting") activityCode = "HW";
        else if (
          actLower === "drone update area" ||
          actLower === "drone lifting" ||
          actLower === "ceremony sail away"
        )
          activityCode = "DR";
        else if (actLower === "back up data") activityCode = "DR";
        else if (actLower === "network" || actLower === "connect share folder")
          activityCode = "NW";
        else if (
          actLower === "weekly safety talk" ||
          actLower === "stand by meeting" ||
          actLower === "stand by sunday" ||
          actLower === "other"
        )
          activityCode = "OT";
        const activityName = act;
        const subjectForExport = ticket.is_assignment
          ? ticket.subject || activityName || "No Subject"
          : ticket.subject || "No Subject";
        ...
        return {
          ...
          user_email:
            (cachedUser && cachedUser.email) || ticket.user_email || "",
  ...
  loadAdminInfo() {
    if (this.adminUser) {
      ...
      if (adminNameEl)
        adminNameEl.textContent = this.adminUser.name || this.adminUser.email;
      if (adminRoleEl) adminRoleEl.textContent = this.adminUser.role || "Admin";
      if (welcomeAdminEl)
        welcomeAdminEl.textContent =
          this.adminUser.name || this.adminUser.email;
    }
  }
  ...
  updateStats() {
    ...
    const openTickets = this.filteredTickets.filter(
      (ticket) => ticket.status === "Open"
    ).length;
    const inProgressTickets = this.filteredTickets.filter(
      (ticket) => ticket.status === "In Progress"
    ).length;
    const resolvedTickets = this.filteredTickets.filter(
      (ticket) => ticket.status === "Resolved"
    ).length;
    const highPriorityTickets = this.filteredTickets.filter(
      (ticket) => ticket.priority === "High"
    ).length;
    const myTickets = this.tickets.filter((ticket) =>
      this.isAssignedToCurrentAdmin(ticket)
    ).length;
  ...
  async getAssignedAdminInfo(ticket) {
    try {
      const actionByInfo = ticket.action_by
        ? await this.getAdminInfo(ticket.action_by)
        : null;
      const assignedToInfo = ticket.assigned_to
        ? await this.getAdminInfo(ticket.assigned_to)
        : null;
  ...
  async getAdminDisplayInfo(adminUid) {
    try {
      ...
      if (adminDoc.exists()) {
        const adminData = adminDoc.data();
        const displayInfo = `${adminData.name || "Unknown Admin"} (${
  ...
  async getUserDisplayInfo(ticket) {
    try {
      ...
      if ((ticket.assignment_activity || "").trim()) {
        let creatorName = ticket.created_by_name || nameFallback;
        try {
          if (
            (!creatorName || creatorName === nameFallback) &&
            ticket.created_by_uid
          ) {
            if (window.userCache && window.userCache[ticket.created_by_uid]) {
              const u = window.userCache[ticket.created_by_uid];
              creatorName = u.full_name || u.name || creatorName;
            } else {
              const udoc = await getDoc(
                doc(this.db, "users", ticket.created_by_uid)
              );
              if (udoc.exists()) {
                const ud = udoc.data();
                if (!window.userCache) window.userCache = {};
                window.userCache[ticket.created_by_uid] = ud;
                creatorName = ud.full_name || ud.name || creatorName;
              } else {
                const adoc = await getDoc(
                  doc(this.db, "admins", ticket.created_by_uid)
                );
                if (adoc.exists()) {
                  const ad = adoc.data();
                  if (!window.adminCache) window.adminCache = {};
                  window.adminCache[ticket.created_by_uid] = ad;
                  creatorName = ad.full_name || ad.name || creatorName;
                }
              }
            }
          }
        ...
        if ((creatorName || "").toLowerCase() === "it") creatorName = "IT";
        return {
          name: creatorName,
      ...
      if (ticket.user_id) {
        if (window.userCache && window.userCache[ticket.user_id]) {
          ...
          return {
            name: u.full_name || nameFallback,
            email: u.email || emailFallback,
            department: u.department || deptFallback,
            phone: u.phone || phoneFallback,
          };
        }
        ...
        if (userDoc.exists()) {
          ...
          return {
            name: userData.full_name || nameFallback,
            email: userData.email || emailFallback,
            department: userData.department || deptFallback,
            phone: userData.phone || phoneFallback,
          };
        }
      }
      return {
      ...
      return {
        name: ticket.user_name || ticket.name || "Unknown",
        email: ticket.user_email || "",
        department: ticket.user_department || "N/A",
  ...
  async showUpdateFormModalSimple(ticket, userDisplay, permissions) {
    try {
      ...
      const modalHTML = `
            <div class="modal-content large" style="max-width: 700px;">
                <div class="modal-header">
                    <h3><i class="fas fa-edit"></i> Update Ticket: ${
                      ticket.code
                    }</h3>
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-grid" style="grid-template-columns: 1fr; gap: 0.75rem;">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="updateSubject" class="form-control" value="${this.escapeHtml(
                                      ticket.subject
                                    )}" required>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" id="updateInventory" class="form-control" value="${this.escapeHtml(
                                      ticket.inventory || ""
                                    )}" placeholder="e.g., IT-001, LAP-002">
                                </div>
                                <div class="form-group">
                                    ...
                                    <select id="updateDevice" class="form-control">
                                        ...
                                        <option value="PC Hardware" ${
                                          ticket.device === "PC Hardware"
                                            ? "selected"
                                            : ""
                                        }>PC Hardware</option>
                                        <option value="PC Software" ${
                                          ticket.device === "PC Software"
                                            ? "selected"
                                            : ""
                                        }>PC Software</option>
                                        <option value="Laptop" ${
                                          ticket.device === "Laptop"
                                            ? "selected"
                                            : ""
                                        }>Laptop</option>
                                        <option value="Printer" ${
                                          ticket.device === "Printer"
                                            ? "selected"
                                            : ""
                                        }>Printer</option>
                                        <option value="Network" ${
                                          ticket.device === "Network"
                                            ? "selected"
                                            : ""
                                        }>Network</option>
                                        <option value="Projector" ${
                                          ticket.device === "Projector"
                                            ? "selected"
                                            : ""
                                        }>Projector</option>
                                        <option value="Backup Data" ${
                                          ticket.device === "Backup Data"
                                            ? "selected"
                                            : ""
                                        }>Backup Data</option>
                                        <option value="Others" ${
                                    ...
                                    <select id="updateActivity" class="form-control">
                                        ...
                                        <option value="Ceremony Sail Away" ${
                                          ...
                                          "Ceremony Sail Away"
                                            ? "selected"
                                            : ""
                                        }>Ceremony Sail Away</option>
                                        <option value="Deliver" ${
                                          (ticket.assignment_activity ||
                                            ticket.activity) === "Deliver"
                                            ? "selected"
                                            : ""
                                        }>Deliver</option>
                                        ...
                                        <option value="Setup Meeting" ${
                                          (ticket.assignment_activity ||
                                            ticket.activity) === "Setup Meeting"
                                            ? "selected"
                                            : ""
                                        ...
                                        <option value="Drone Update Area" ${
                                          ...
                                          "Drone Update Area"
                                            ? "selected"
                                            : ""
                                        }>Drone Update Area</option>
                                        <option value="Drone Lifting" ${
                                          (ticket.assignment_activity ||
                                            ticket.activity) === "Drone Lifting"
                                            ? "selected"
                                            : ""
                                        }>Drone Lifting</option>
                                        <option value="Stand By Meeting" ${
                                          ...
                                          "Stand By Meeting"
                                            ? "selected"
                                            : ""
                                        }>Stand By Meeting</option>
                                        <option value="Stand By Sunday" ${
                                          (ticket.assignment_activity ||
                                            ticket.activity) ===
                                          "Stand By Sunday"
                                            ? "selected"
                                            : ""
                                        }>Stand By Sunday</option>
                                        <option value="Back Up Data" ${
                                          (ticket.assignment_activity ||
                                            ticket.activity) === "Back Up Data"
                                            ? "selected"
                                            : ""
                                        }>Back Up Data</option>
                                        <option value="Connect Share Folder" ${
                                          ...
                                          "Connect Share Folder"
                                            ? "selected"
                                            : ""
                                        }>Connect Share Folder</option>
                                        <option value="Software Config" ${
                                          (ticket.assignment_activity ||
                                            ticket.activity) ===
                                          "Software Config"
                                            ? "selected"
                                            : ""
                                        }>Software Config</option>
                                        <option value="Install IT Standard Apps" ${
                                          ...
                                          "Install IT Standard Apps"
                                            ? "selected"
                                            : ""
                                        }>Install IT Standard Apps</option>
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="updateLocation" class="form-control" required>
                                        ...
                                        <option value="Blue Office" ${
                                          ticket.location === "Blue Office"
                                            ? "selected"
                                            : ""
                                        }>Blue Office</option>
                                        <option value="Clinic" ${
                                          ticket.location === "Clinic"
                                            ? "selected"
                                            : ""
                                        }>Clinic</option>
                                        <option value="Control Room" ${
                                          ticket.location === "Control Room"
                                            ? "selected"
                                            : ""
                                        }>Control Room</option>
                                        <option value="Dark Room" ${
                                          ticket.location === "Dark Room"
                                            ? "selected"
                                            : ""
                                        }>Dark Room</option>
                                        <option value="Green Office" ${
                                          ticket.location === "Green Office"
                                            ? "selected"
                                            : ""
                                        }>Green Office</option>
                                        <option value="HRD" ${
                                        ...
                                        <option value="HSE Yard" ${
                                          ticket.location === "HSE Yard"
                                            ? "selected"
                                            : ""
                                        }>HSE Yard</option>
                                        <option value="IT Server" ${
                                          ticket.location === "IT Server"
                                            ? "selected"
                                            : ""
                                        }>IT Server</option>
                                        <option value="IT Store" ${
                                          ticket.location === "IT Store"
                                            ? "selected"
                                            : ""
                                        }>IT Store</option>
                                        <option value="Multi Purposes Building" ${
                                          ...
                                          "Multi Purposes Building"
                                            ? "selected"
                                            : ""
                                        }>Multi Purposes Building</option>
                                        <option value="Red Office" ${
                                          ticket.location === "Red Office"
                                            ? "selected"
                                            : ""
                                        }>Red Office</option>
                                        <option value="Security" ${
                                          ticket.location === "Security"
                                            ? "selected"
                                            : ""
                                        }>Security</option>
                                        <option value="Store 1" ${
                                          ticket.location === "Store1"
                                            ? "selected"
                                            : ""
                                        }>Store 1</option>
                                        <option value="Store 2" ${
                                          ticket.location === "Store2"
                                            ? "selected"
                                            : ""
                                        }>Store 2</option>
                                        <option value="Store 3" ${
                                          ticket.location === "Store3"
                                            ? "selected"
                                            : ""
                                        }>Store 3</option>
                                        <option value="Store 4" ${
                                          ticket.location === "Store4"
                                            ? "selected"
                                            : ""
                                        }>Store 4</option>
                                        <option value="Store 5" ${
                                          ticket.location === "Store5"
                                            ? "selected"
                                            : ""
                                        }>Store 5</option>
                                        <option value="Store 6" ${
                                          ticket.location === "Store6"
                                            ? "selected"
                                            : ""
                                        }>Store 6</option>
                                        ...
                                        <option value="White Office 2nd Fl" ${
                                          ...
                                          "White Office 2nd Fl"
                                            ? "selected"
                                            : ""
                                        }>White Office 2nd Floor</option>
                                        <option value="White Office 3rd Fl" ${
                                          ticket.location ===
                                          "White Office 3rd Fl"
                                            ? "selected"
                                            : ""
                                        }>White Office 3rd Floor</option>
                                        <option value="Welding School" ${
                                          ticket.location === "Welding School"
                                            ? "selected"
                                            : ""
                                        }>Welding School</option>
                                        <option value="Workshop9" ${
                                          ticket.location === "Workshop9"
                                            ? "selected"
                                            : ""
                                        }>Workshop 9</option>
                                        <option value="Workshop10" ${
                                          ticket.location === "Workshop10"
                                            ? "selected"
                                            : ""
                                        }>Workshop 10</option>
                                        <option value="Workshop11" ${
                                          ticket.location === "Workshop11"
                                            ? "selected"
                                            : ""
                                        }>Workshop 11</option>
                                        <option value="Workshop12" ${
                                          ticket.location === "Workshop12"
                                            ? "selected"
                                            : ""
                                        }>Workshop 12</option>
                                        <option value="Yard" ${
                                          ticket.location === "Yard"
                                            ? "selected"
                                            : ""
                                        }>Yard</option>
                                        <option value="Rest Area" ${
                                          ticket.location === "Rest Area"
                                            ? "selected"
                                            : ""
                                        }>Rest Area</option>
                                        <option value="Lainlain" ${
                                          ticket.location === "Lainlain"
                                            ? "selected"
                                            : ""
                                ...
                                <div class="form-group" style="margin-top: 1rem;">
                                    ...
                                    <textarea id="updateAdminNotes" class="form-control" rows="3" placeholder="Admin notes...">${this.escapeHtml(
                                      ticket.note || ""
                                    )}</textarea>
                                </div>
                            </div>
                        ...
                        <div class="form-section" style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid var(--border);">
                            ...
                            <div class="form-grid" style="grid-template-columns: 1fr; gap: 0.75rem;">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="updateUserName" class="form-control" value="${this.escapeHtml(
                                      userDisplay.name
                                    )}" ${
        isSuperAdmin ? "" : "disabled"
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="email" id="updateUserEmail" class="form-control" value="${this.escapeHtml(
                                      userDisplay.email || ""
                                    )}" ${
        isSuperAdmin ? "" : "disabled"
      } required>
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="updateUserDepartment" class="form-control" ${
                                      isSuperAdmin ? "" : "disabled"
                                    } required>
                                        <option value="">Select Department</option>
                                        <option value="Admin" ${
                                          userDisplay.department === "Admin"
                                            ? "selected"
                                            : ""
                                        }>Admin</option>
                                        <option value="Civil" ${
                                          userDisplay.department === "Civil"
                                            ? "selected"
                                            : ""
                                        }>Civil</option>
                                        <option value="Clinic" ${
                                          userDisplay.department === "Clinic"
                                            ? "selected"
                                            : ""
                                        }>Clinic</option>
                                        <option value="Client" ${
                                          userDisplay.department === "Client"
                                            ? "selected"
                                            : ""
                                        }>Client</option>
                                        ...
                                        <option value="DC" ${
                                          userDisplay.department === "DC"
                                            ? "selected"
                                            : ""
                                        }>Dimentional Control (DC)</option>
                                        ...
                                        <option value="Engineer" ${
                                          userDisplay.department === "Engineer"
                                            ? "selected"
                                            : ""
                                        }>Engineering</option>
                                        <option value="Finance" ${
                                          userDisplay.department === "Finance"
                                            ? "selected"
                                            : ""
                                        }>Finance</option>
                                        <option value="HSE" ${
                                          userDisplay.department === "HSE"
                                            ? "selected"
                                            : ""
                                        }>HSE</option>
                                        <option value="HR" ${
                                          userDisplay.department === "HR"
                                            ? "selected"
                                            : ""
                                        }>Human Resources (HRD)</option>
                                        <option value="IT" ${
                                          userDisplay.department === "IT"
                                            ? "selected"
                                            : ""
                                        }>IT</option>
                                        ...
                                        <option value="PGA" ${
                                          userDisplay.department === "PGA"
                                            ? "selected"
                                            : ""
                                        }>PGA</option>
                                        <option value="Planner" ${
                                          userDisplay.department === "Planner"
                                            ? "selected"
                                            : ""
                                        }>Planner</option>
                                        ...
                                        <option value="QC" ${
                                          userDisplay.department === "QC"
                                            ? "selected"
                                            : ""
                                        }>Quality Control (QC)</option>
                                        <option value="Vendor" ${
                                          userDisplay.department === "Vendor"
                                            ? "selected"
                                            : ""
                                        }>Vendor</option>
                                        <option value="Warehouse" ${
                                          userDisplay.department === "Warehouse"
                                            ? "selected"
                                            : ""
                                        }>Warehouse</option>
                                        <option value="Lainlain" ${
                                          userDisplay.department === "Lainlain"
                                            ? "selected"
                                            : ""
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" id="updateUserPhone" class="form-control" value="${this.escapeHtml(
                                      userDisplay.phone ||
                                        ticket.user_phone ||
                                        ""
                                    )}" ${
      ...
      updateModal.addEventListener("click", (e) => {
        const isBackdrop =
          e.target === updateModal || !e.target.closest(".modal-content");
        if (isBackdrop) {
  ...
  async handleTicketUpdateSimple(ticketId) {
    try {
      const submitBtn = document.querySelector(
        '#updateTicketForm button[type="submit"]'
      );
      ...
      if (missingElements.length > 0) {
        throw new Error(
          `Required fields not found: ${missingElements.join(
            ", "
      ...
      try {
        ...
        if (
          ...
          currentData.message.includes("Location:")
        ) {
          const replaced = currentData.message.replace(
            /(Location:\s*)([^|]+)(?=\s*\|?|$)/,
            `$1${newLoc}`
          );
        ...
        if (
          typeof currentData.assignment_message === "string" &&
          currentData.assignment_message.includes("Location:")
        ) {
          const replacedAssign = currentData.assignment_message.replace(
            /(Location:\s*)([^|]+)(?=\s*\|?|$)/,
            `$1${newLoc}`
          );
        ...
        if (
          currentData.is_assignment &&
          typeof currentData.assignment_location !== "undefined"
        ) {
      ...
      const isSuperAdmin =
        this.adminUser && this.adminUser.role === "Super Admin";
  ...
  async handleLogout() {
    try {
      ...
      try {
        ...
        console.warn(
          "Logout via service failed, continuing client-side cleanup:",
          error?.message || error
        );
      }
