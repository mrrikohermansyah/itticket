...
try {
    ...
    const getCode = function (value, map) {
        ...
        const found = keys.find(k => v.toLowerCase().includes(k.toLowerCase()) || k.toLowerCase().includes(v.toLowerCase()));
        return found ? map[found] : v.substring(0, 3).toUpperCase();
    };
    window.generateTicketId = function (department, device, location, firestoreId, activity) {
        ...
        const dateSeg = yy + mm;
        const idSeg = (typeof firestoreId === 'string' && firestoreId.length >= 3) ? firestoreId.slice(-3).toUpperCase() : 'XXX';
