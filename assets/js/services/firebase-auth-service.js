...
class FirebaseAuthService {
  constructor() {
    ...
    this.validateUserProfileUpdates =
      this.validateUserProfileUpdates.bind(this);
    this.capitalizeFirstLetter = this.capitalizeFirstLetter.bind(this);
    this.updateUserProfile = this.updateUserProfile.bind(this);
    this.updateUserTicketsInFirestore =
      this.updateUserTicketsInFirestore.bind(this);
    this.validateAndCleanUserUpdates =
      this.validateAndCleanUserUpdates.bind(this);
  ...
  async loginAdmin(email, password) {
    try {
          ...
          if (!adminData) {
            try {
              ...
              if (!byEmail.empty) {
                ...
                try {
                  await updateDoc(doc(db, "admins", found.id), {
                    migrated_to: user.uid,
                    last_updated: new Date().toISOString(),
                  });
                } catch (_) {}
                adminData = data;
      ...
      if (!adminData) {
        ...
        try {
          const settingsSnap = await getDoc(
            doc(db, "secure_settings", "allowed_admin_emails")
          );
          if (settingsSnap.exists()) {
            const s = settingsSnap.data();
            const allowAll = !!s.allow_all;
            const list = Array.isArray(s.emails)
              ? s.emails.map((e) => (e || "").toLowerCase())
              : [];
        ...
        if (isAllowed) {
          ...
          if (wasSoftDeleted) {
            throw new Error(
              "Admin access not granted. Please contact administrator."
            );
          }
          const nextUser = {
      ...
      try {
        const desiredRole = await this.resolveRoleForEmail(
      ...
      try {
        const resolved = await this.resolveRoleForEmail(
          adminData?.email || email
        );
        if (
          resolved === "Super Admin" &&
          (adminData?.role || "admin") !== "Super Admin"
        ) {
          try {
            await updateDoc(doc(db, "admins", user.uid), {
              role: "Super Admin",
  ...
  async migrateUserToAdmin(uid, userData) {
    try {
      await setDoc(doc(db, "admins", uid), {
        name:
          userData.full_name || userData.name || userData.email.split("@")[0],
        email: userData.email,
    ...
    try {
      ...
      const nextUser = {
        ...
        role:
          currentRole !== "user"
            ? currentRole
            : await this.resolveRoleForEmail(email),
        is_active: true,
        updated_at: new Date().toISOString(),
        created_at: base.created_at || new Date().toISOString(),
      };
  ...
  async resolveRoleForEmail(email) {
    try {
      ...
      try {
        ...
        if (settingsSnap.exists()) {
          const s = settingsSnap.data();
          const superList = Array.isArray(s.super_admin_emails)
            ? s.super_admin_emails.map((v) => (v || "").toLowerCase())
            : [];
  ...
  async updateUserProfile(uid, updates) {
    try {
      ...
      return {
        success: true,
        message:
          "Profile updated successfully! All related tickets have been updated.",
        user: { uid, ...updates },
      };
    } catch (error) {
      ...
      try {
        ...
        if (authMethods.length > 0) {
          ...
          return {
            success: false,
            message:
              "User already exists in authentication. Please use different email or contact administrator.",
          };
        } else {
